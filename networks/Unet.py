...
class ChannelAttention(nn.Module):
    def __init__(self,in_channels,reduction=16):
        ...
        self.anatomy_scale = nn.Parameter(torch.ones(1))
    ...
    def forward(self, x):
        ...
        gap_att = self.fc(gap)
        gmp_att = self.fc(gmp)
        combined_att = (gap_att + gmp_att) / 2

        # 解剖约束：防止过度抑制医学特征
        att_weight = 0.5 + self.anatomy_scale * (combined_att - 0.5)

        return x * att_weight.view(b, c, 1, 1)
