...
"""
Invariant Risk Minimization (IRM)：引入因果不变性约束进baseline
"""
...
class IRMSegmentationModel(nn.Module):
    """IRM分割模型"""
...
class IRMLoss(nn.Module):
    ...
    def __init__(self, num_classes=2, irm_lambda=1.0,dice_weight = 0.5,ce_weight = 0.5):
        super().__init__()
    ...
    def compute_irm_penalty(self, logits, targets, classifier_weight):
        """计算IRM梯度惩罚"""
        # 创建虚拟的w=1.0分类器权重
        dummy_w = torch.ones_like(classifier_weight)
        dummy_w.requires_grad_(True)
    ...
    def forward(self, predictions_by_domain, targets_by_domain, classifier_weight):
        """
        ...
        for domain_id in predictions_by_domain:
            logits = predictions_by_domain[domain_id]
            targets = targets_by_domain[domain_id]

            # 分割损失
            #seg_loss = self.seg_loss(logits, targets)
            seg_loss = (self.dice_weight * self.dice_loss(logits, targets)
                        + self.ce_weight * self.ce_loss(logits, targets))
            total_seg_loss += seg_loss

            # IRM梯度惩罚
            irm_penalty = self.compute_irm_penalty(logits, targets, classifier_weight)
            total_irm_penalty += irm_penalty

        # 总损失
        total_loss = total_seg_loss + self.irm_lambda * total_irm_penalty

        return {
            'total_loss': total_loss,
            'seg_loss': total_seg_loss,
            'irm_penalty': total_irm_penalty
        }
...
class IRMTrainer:
    ...
    def __init__(self, model, train_domains=[0, 1, 2], test_domain=3,
                 irm_lambda=1.0, lr=1e-4, device='cuda'):
        ...
        self.criterion = IRMLoss(irm_lambda=irm_lambda)
    ...
    def train_epoch(self, dataloaders: Dict[int, DataLoader]):
        ...
        for batch_idx in range(min_batches):
            ...
            loss_dict = self.criterion(
                predictions_by_domain,
                targets_by_domain,
                self.model.final_classifier.weight
            )
            ...
            #           f'IRM: {loss_dict["irm_penalty"].item():.4f}')
    ...
    def train(self, dataloaders: Dict[int, DataLoader], test_dataloader: DataLoader,
        ...
        epoch_pbar.close()
class AdaptiveIRMTrainer(IRMTrainer):
    """自适应IRM训练器，动态调整λ"""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.initial_irm_lambda = kwargs.get('irm_lambda', 1.0)
        self.warmup_epochs = 40  # 前20个epoch较小的IRM权重
...
def run_leave_one_domain_out(images, masks, domain_labels, num_classes=2):
    ...
    for test_domain in domain_pbar:
        ...
        # 创建模型和训练器
        model = IRMSegmentationModel(num_classes=num_classes)
        ...
        # 训练
        trainer.train(train_dataloaders, test_dataloader, num_epochs=150)
