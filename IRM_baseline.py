...
class MultiLayerIRMTrainer:
    ...
    def __init__(self, model, train_domains=[0, 1, 2], test_domain=3,
        ...
        self.criterion = MultiLayerIRMLoss()
    ...
    def evaluate(self, test_dataloader: DataLoader):
        ...
        with torch.no_grad():
            for batch in pbar:
                ...
                loss = 0.5 * DiceLoss()(outputs, masks) + 0.5 * nn.CrossEntropyLoss()(outputs, masks)
    ...
    def train(self, dataloaders: Dict[int, DataLoader], test_dataloader: DataLoader,
        ...
        for epoch in epoch_pbar:
            ...
            # 训练
            train_loss = self.train_epoch(dataloaders)
            ...
            if epoch % 5 == 0:  # 每5个epoch验证一次
                ...
                print(f'Train Losses - Total: {train_loss:.4f}\t')
...
if __name__ == "__main__":
    ...
    # 示例数据（实际使用时替换为你的真实数据）
    images,masks,domain_labels = read_data(root_dir = 'D:\dev\python\Datasets\Fundus-doFE\Fundus',domains = [0,1,2,3])
    # 运行留一域验证
