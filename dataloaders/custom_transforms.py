...
def to_multilabel(pre_mask, classes = 2):
    ...
    mask[pre_mask == 1] = [0, 1] #标签为1（视盘）设置为0，1
    mask[pre_mask == 2] = [1, 1]#标签为2（视杯）设置为1，1（视盘包含了视杯）
    return mask #本质上是将mask转化为了one-hot的格式，但是只有视杯和视盘两个类别
...
class Normalize_tf(object):
    ...
    def __call__(self, sample):
        ...
        __mask[_mask == 0] = 2 #视杯设置标签为2
        __mask[_mask == 255] = 0 #背景设置标签为0
        __mask[_mask == 128] = 1 #视盘设置标签为1

        mask = to_multilabel(__mask) #将原本为单通道的多类别标签转换为多通道标签（类似于ont-hot）
        sample['image'] = img
        sample['label'] = mask
        return sample
